...
BOOL CBombDlg::OnInitDialog()
{
	...
	m_MinMalGrp = 0; 
	m_MaxMalGrp = 2;
	m_Instanserr = 2;			// Antal instanser
	m_DelayClp = 80;
...
void CBombDlg::las_oddsfrekvens()
{
	...
	if(m_speltyp == 0 || m_speltyp == 3 || m_speltyp == 6 || m_speltyp ==11 || m_speltyp ==12)
...
float CBombDlg::berakna_sannt_odds_for_en_kombination(int r1,int r2,int match,bool kvot)
{
	...
	if(r1 > r2)
	{
		...
		if(oddstyp == 1 && index == 37)
			...
			max_funna_odds = 3;
		else if(oddstyp == 2 && index == 23)
			max_funna_odds = 3;

		// Fick fel "blocks nested too deeply" GB
		if(oddstyp == 2 && index == 25)
			max_funna_odds = 5;
...
void CBombDlg::OnEditPaste() 
{
	...
	// Hocky/Bandy med 2 matcher -->
	else if (sret==6 && (m_speltyp==1 || m_speltyp==2 || m_speltyp==4 || m_speltyp==7 || m_speltyp==13))
	{
		m_m1_ett	= H1;
		m_m1_kryss	= O1;
		m_m1_tva	= B1;
...
void CBombDlg::LasOddsiSpelFonster() 
{
	...
	if (m_speltyp == 1 || m_speltyp == 2  || m_speltyp == 13)
	{ 
		  ...
		  for (int j= 0; j<MAXRES; j++)
		  {
				...
				||  (m_OddsOspelActive && (minnoodds < m_OddsOspelade))))
			{
				...
				if (!UNIBETSPEL(m_speltyp))
					...
					scrn.LasResUnibet(hChild, &bong2, ANTALMATCHER(m_speltyp), 0);
				}
...
#define XOR(a,b) ((a) && !(b) || !(a) && (b))
	...
	int xh1 = TRE(m_speltyp) ? UB_XH1M3 : UB_XH1M2;
	int xh2 = TRE(m_speltyp) ? UB_XH2M3 : UB_XH2M2;
	...
	for (i=0; i<MAXRES; i++)
	{
		if (XOR(ArMalMed(i, bong1->h1list, bong1->h1index), ArMalMed(i, bong2->h1list, bong2->h1index))) 
		{
			MouseClickLeft(XPOS(rc->left + xh1), YPOS(rc->top+ypos + UB_YTOP + UB_ROWDIFF*i));
			AntalKryss ++;
			::Sleep(SleepTime);
		}
	}
	for (i=0; i<MAXRES; i++)
	{
		if (XOR(ArMalMed(i, bong1->b1list, bong1->b1index), ArMalMed(i, bong2->b1list, bong2->b1index))) 
		{
			MouseClickLeft(XPOS(rc->left + xh1 + UB_XWD), YPOS(rc->top+ypos + UB_YTOP + UB_ROWDIFF*i));
			AntalKryss ++;
			::Sleep(SleepTime);
		}
	}
	for (i=0; i<MAXRES; i++)
		if (XOR(ArMalMed(i, bong1->h2list, bong1->h2index), ArMalMed(i, bong2->h2list, bong2->h2index))) 
		{
			MouseClickLeft(XPOS(rc->left + xh2), YPOS(rc->top+ypos + UB_YTOP + UB_ROWDIFF*i));
			AntalKryss ++;
			::Sleep(SleepTime);
		}
	for (i=0; i<MAXRES; i++)
		if (XOR(ArMalMed(i, bong1->b2list, bong1->b2index), ArMalMed(i, bong2->b2list, bong2->b2index))) 
		{
			MouseClickLeft(XPOS(rc->left + xh2 + UB_XWD), YPOS(rc->top+ypos + UB_YTOP + UB_ROWDIFF*i));
			AntalKryss ++;
			::Sleep(SleepTime);
		}
	...
	if (TRE(m_speltyp)|| m_speltyp==11) 
	{
		for (i=0; i<MAXRES; i++)
			if (XOR(ArMalMed(i, bong1->h3list, bong1->h3index), ArMalMed(i, bong2->h3list, bong2->h3index))) 
			{
				MouseClickLeft(XPOS(rc->left + UB_XH3M3), YPOS(rc->top+ypos + UB_YTOP + UB_ROWDIFF*i));
				AntalKryss ++;
				::Sleep(SleepTime);
			}
		for (i=0; i<MAXRES; i++)
			if (XOR(ArMalMed(i, bong1->b3list, bong1->b3index), ArMalMed(i, bong2->b3list, bong2->b3index))) 
			{
				MouseClickLeft(XPOS(rc->left + UB_XH3M3 + UB_XWD), YPOS(rc->top+ypos + UB_YTOP + UB_ROWDIFF*i));
				AntalKryss ++;
				::Sleep(SleepTime);
			}
	}
	...
	for (int retry = 0; retry<10 && diff; retry++)
	{
		scrn.LasResUnibet(HittaSpelFonster(100), &kryssad, ANTALMATCHER( m_speltyp ), 0);
		diff = false;
		for (int i=0; i<MAXRES && !diff; i++)
		{
			if (bong1->h1index != kryssad.h1index  
